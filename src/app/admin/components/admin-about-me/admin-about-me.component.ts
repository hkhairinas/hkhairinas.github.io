...
export class AdminAboutMeComponent implements OnInit {
  ...
  public upload(event, id: string) {
    this.uploadPercent = this.uploadImage(event, id);
  }
      ...
      .pipe(
        finalize(() => {
          fileRef
            ...
            .toPromise()
            .then((data) =>
              this.saveDownloadUrl(id, data, this.profileCollectionRef)
            )
  ...
  public saveDownloadUrl(
    documentId: string,
    downloadUrl: string,
    profileColRef: AngularFirestoreCollection
  ) {
    this.dataService.saveDownloadURL(documentId, downloadUrl, profileColRef);
  }

  public editProfile($event) {
    const { firstName, lastName, jobTitle, startYear, company, aboutMe } =
      $event;
    this.profileCollectionRef
      .doc(this.itemId)
      .update({ firstName, lastName, jobTitle, startYear, company, aboutMe });
  }
